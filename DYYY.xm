...
- (id)timestampLabel {
    UILabel *label = %orig;
    // 添加一个控制换行的布尔值，这里假设从NSUserDefaults获取，你可以根据实际情况调整获取方式
    BOOL enableLineBreakForLocation = [[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYisEnableAreaHH"]; 
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYisEnableArea"]) {
        ...
        if (cityCode.length > 0) {
            ...
            if (!cityName || cityName.length == 0) {
                ...
                if (!cachedData) {
                    NSString *cachesDir = [NSSearchPathForDirectoriesInDomains(NSCacheDirectory, NSUserDomainMask, YES) firstObject];
                ...
                if (cachedData) {
                    ...
                    if (countryName.length > 0) {
                        if (adminName1.length > 0 && localName.length > 0 && ![countryName isEqualToString:@"中国"] && ![countryName isEqualToString:localName]) {
                            // 根据开关决定是否换行
                            if (enableLineBreakForLocation) {
                                displayLocation = [NSString stringWithFormat:@"%@\n%@\n%@", countryName, adminName1, localName];
                            } else {
