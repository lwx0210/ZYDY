    ...
    if ([[NSUserDefaults standardUserDefaults] boolForKey:@"DYYYisEnableArea"]) {
        ...
        if (cityCode.length > 0) {
            ...
            if (!cityName || cityName.length == 0) {
                ...
                NSString *prefix = cityCode.length >= 2 ? [cityCode substringToIndex:2] : @"";
                ...
                if ([@[@"81", @"82", @"71"] containsObject:prefix]) {
                    ...
                    if (provinceName.length > 0 && cityCode.length >= 2) {
                        [components addObject:provinceName];
                    }

                    if (cityName.length > 0 && cityCode.length >= 4 && ![cityName isEqualToString:provinceName]) {
                ...
                if (components.count > 0) {
                    ...
                    if ([prefix isEqualToString:@"71"] && [cityName containsString:@"福建省"]) {
